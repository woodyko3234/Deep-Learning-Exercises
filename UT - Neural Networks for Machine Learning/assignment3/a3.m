...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  derative_w = class_prob - data.targets; % for the other w, no change, for the wi, minus - 1  number of class * number of data cases
  derative_hid = model.hid_to_class'*derative_w;
  derative_hidin =derative_hid.*(hid_output.*(1-hid_output));
  
   
  num = size(data.inputs,2);
  %classification_loss = -mean(sum(log_class_prob .* data.targets, 1)); % select the right log class probability using that sum; then take the mean over all data cases.
  %wd_loss = sum(model_to_theta(model).^2)/2*wd_coefficient; % weight decay loss. very straightforward: E = 1/2 * wd_coeffecient * theta^2
  %ret = classification_loss + wd_loss;
  ...
  ret.input_to_hid = derative_hidin*data.inputs'./num + wd_coefficient*model.input_to_hid;
  
  ret.hid_to_class = derative_w * hid_output'./num+wd_coefficient*model.hid_to_class;
